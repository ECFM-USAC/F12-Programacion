...
int n, objetivo;
std::cout << "Ingrese el número de elementos en la lista: ";
std::cin >> n;
std::cout << "Ingrese los elementos de la lista (en orden ascendente): ";
for (int i = 0; i < n; i++) {
    int elemento;
    std::cin >> elemento;
    lista.push_back(elemento);
}
std::cout << "Ingrese el objetivo a buscar: ";
std::cin >> objetivo;
int indice = busqueda_de_dos_en_dos(lista, n, objetivo);
if (indice != -1) {
    std::cout << "El objetivo " << objetivo << " se encuentra en el índice " << indice << std::endl;
} else {
    std::cout << "El objetivo " << objetivo << " no se encuentra en la lista." << std::endl;
}
return 0;
}